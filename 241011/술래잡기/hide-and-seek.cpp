...
int n, m, h, k, cur_turn, dir_boss, prev_boss, cnt_boss, score_boss;
...
void move_boss() {
    ...
    if (boss == make_pair(1, 1)) { // 역방향 전환
        rpt_boss = true, prev_boss = n, cnt_boss = n - 1, dir_boss = 2, rev_boss = true;
    }

    if (boss == make_pair((n + 1) / 2, (n + 1) / 2)) { // 순방향 전환
